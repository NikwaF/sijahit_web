...
class Pos extends CI_Controller{
	...
	public function tambah_data()
	{
		if(adminLoggedIn()){
			...
			$this->form_validation->set_rules('kode_pos', 'Kodepos', 'required|min_length[5]|is_unique[biaya_onkir.kode_pos]');
	...
	{
		$post = $this->input->post(null, TRUE);
		$query = $this->db->query("SELECT * FROM biaya_onkir WHERE kode_pos='$post[kode_pos]'  AND kode_pos != '$post[kode]'");
		if($query->num_rows()>0){
